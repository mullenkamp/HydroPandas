...
#    gaugings = gaugings[gauge_sites2]
    ...
    idx = (sd1a.mon_usage_m3.isnull()) & (sd1a.sd_usage > sd1a.mon_restr_allo_m3)
    sd1a.loc[idx,'sd_usage'] = sd1a.mon_restr_allo_m3.loc[idx]

    """
    if wap_exclusion_list is not None:

        wap_exclusion_list = np.atleast_1d(wap_exclusion_list)
        idx = ~ np.in1d([sd1a.wap],wap_exclusion_list)
        sd1a = sd1a.loc[idx].reset_index()
        """

    if wap_exclusion_list is not None:
        for key in wap_exclusion_list.keys():
            idx = ~ ((sd1a.site == key) & np.in1d([sd1a.wap], wap_exclusion_list[key]))
            sd1a = sd1a.loc[idx].reset_index(drop=True)
