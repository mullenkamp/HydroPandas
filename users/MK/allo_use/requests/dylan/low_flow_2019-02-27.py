...
export1 = 'lf_crc_bands_2019-02-27.csv'
...
crc1 = mssql.rd_sql(server, database, crc_table, ['crc', 'from_date', 'to_date', 'crc_status', 'max_rate_crc', 'feav', 'use_type'], {'crc': both2.crc.unique().tolist()})

both3 = pd.merge(both2, crc1, on=['crc'])

both3.to_csv(os.path.join(export_path, export1), index=False)
