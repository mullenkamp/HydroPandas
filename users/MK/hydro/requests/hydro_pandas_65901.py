...
h2 = hydro().get_data(mtypes=mtypes1, sites=sites1, qual_codes=qual_codes)
...
## Only one site from both mtypes
h4 = h2.sel(mtypes=['flow', 'm_flow'], sites=[69607, 66])
h4
...
malf4, alfs, mis_alfs = h2.malf7d(sites=[65101, 70105], intervals=[10, 20], return_alfs=True)
malf4
alfs
mis_alfs
...
new1, reg = h1.flow_reg(y=[137, 66, 70103], buffer_dis=40000)
malf6 = new1.malf7d()
new1
reg
malf6
...
h1.to_csv(export_csv2, mtypes='m_flow')

### Export a shapfile

## of the gauging sites - combining two functions
h1.sel(mtypes='m_flow').to_shp(export_shp)
...
new2 = hydro().rd_csv(export_csv2, time='time', mtypes='m_flow', sites='site', values='data', dformat='long')
new2
...
## Regressions
plt2, reg2 = h2.plot_reg('flow', 69602, 'm_flow', 137)
reg2
