...
def vis_cc(relative_data_path, meta_data_path):
    ...
    for site in list(itertools.chain(*groups.values())):
        ...
        g = sns.FacetGrid(temp_data, row='sen', sharey=True, sharex=True,row_order=['naturalised', 'current', 'pc5_80_wil_eff'])
        g.map_dataframe(sns.tsplot, time='period', unit='rcm', value='value', ci=95, condition='condition', color={'RCP4.5_tym':'b', 'RCP4.5_low_3_m':'c', 'RCP8.5_tym':'r', 'RCP8.5_low_3_m':'orange'})
...
def vis_eco_min_flows(relitive_data_path):
...
def vis_relibability(relitive_data_path):
