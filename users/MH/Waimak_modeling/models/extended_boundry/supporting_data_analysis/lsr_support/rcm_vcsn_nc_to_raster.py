...
def visualise_regress_data_all(outdir):
    ...
    for var in ['slope', 'intercept', 'r_value', 'std_err']:
        maxes, mins = [], []
        for path in paths:
            dataset = nc.Dataset(path)
            data = np.array(dataset.variables[var])
...
def plot_example_sites(outdir): #todo should be good to run
    shp_sites = gpd.read_file(r"P:\Groundwater\Waimakariri\Groundwater\Numerical GW model\Model simulations and results\ex_bd_va\niwa_data_explore\montly_nc_data\regressions\sites.shp")
    sites = {name:(geom.x,geom.y) for name,geom in shp_sites.loc[:,['site_name','geometry']].itertuples(False,None)}
    data = {}
    paths = glob(
        r"P:\Groundwater\Waimakariri\Groundwater\Numerical GW model\Model simulations and results\ex_bd_va\niwa_data_explore\montly_nc_data\*-*.nc")
    for path in paths:
    ...
    vcsn_path = r"P:\Groundwater\Waimakariri\Groundwater\Numerical GW model\Model simulations and results\ex_bd_va\niwa_data_explore\montly_nc_data\VirtualClimate_monthly_pe.nc"
    vcsn_data = nc.Dataset(vcsn_path)
    vcsn = vcsn_data.variables['pe']
    lats = vcsn_data.variables['latitude']
    lons = vcsn_data.variables['longitude']
    ...
    for site, (lat, lon) in sites.items():
        ...
        for ax, (m, dat) in zip(axs.flatten(), data.items()):
            rcm_site = dat[:, lat_idx, lon_idx]
            # make a ts plot
            axts.plot(time, rcm_site, c=colors[m], label=m)
            axts.plot(time, vcs_for_site, c='r', label=m)
            axts.legend()
            axts.set_ylabel('pet mm/day')
            axts.set_ylabel('time')

            # make a linear regression plot
            slope, intercept, r_value, p_value, std_err = stats.linregress(rcm_site, vcs_for_site)
            ax.plot(rcm_site, vcs_for_site, c=colors[m], marker='.', label=m)
            abline(slope,intercept,ax,colors[m])
            ax.set_title(m)
            ax.set_ylabel = 'vcsn'
            ax.set_xlabel = 'rcm'
        fig.suptitle(site)
        fig.savefig(os.path.join(outdir, '{}_ts_plot.png'.format(site)))
        fig2.suptitle(site)
        fig.savefig(os.path.join(outdir, '{}_reg_plot.png'.format(site)))
