...
def fao_eto(df, z_msl=500, lat=-43.6, lon=172, TZ_lg=173, z_u=2, time_int='days', max_ETo=15, min_ETo=0, fill=False, maxgap=15, export=False, export_path='ETo.csv'):
    ...
    The input data must be a DataFrame with specific column names accoding to the met parameter. The column names should be any of:
        'R_n' -- Net radiation (MJ/m2)\n
        'R_s' -- Incoming shortwave radiation (MJ/m2)\n
        'G' -- Net soil heat flux (MJ/m2)\n
        'T_min' -- Minimum Temperature (deg C)\n
        'T_max' -- Maximum Temperature (deg C)\n
        'T_mean' -- Mean Temperature (deg C)\n
        'T_dew' -- Dew point temperature (deg C)\n
        'RH_min' -- Minimum relative humidity\n
        'RH_max' -- Maximum relative humidity\n
        'RH_mean' -- Mean relative humidity\n
        'n_sun' -- Number of sunshine hours per day\n
        'U_z' -- Wind speed at height z (m/s)\n
        'P_atmos' -- Atmospheric pressure (kPa)

    Parameter estimation values refer to the quality level of the input parameters into the ETo equations. Where a 0 (or nothing) refers to no necessary parameter estimation (measurement data was available), while a 1 refers to parameters that have the best input estimations and up to a value of 3 is the worst. Starting from the right, the first value refers to U_z, the second value refers to G, the third value refers to R_n, the fourth value refers to R_s, the fifth value refers to e_a, the sixth value refers to T_mean, the seventh value refers to P_atmos.

    Arguments:\n
    df -- DataFrame of met data.\n
    z_msl -- Elevation of the met station above mean sea level (m) (only needed if P_atmos is not in df).\n
    lat -- The latitude of the met station (dec deg) (only needed if R_s or R_n are not in df).n\
    lon -- The longitude of the met station (dec deg) (only needed if calculating ETo hourly).\n
    TZ_lg -- The longitude of the center of the time zone (dec deg) (only needed if calculating ETo hourly).\n
    z_u -- The height of the wind speed measurement (m).\n
    time_int -- The time interval of the input and output (either 'days' or 'hours').\n
    max_ETo -- The max realistic value of ETo (mm).\n
    min_ETo -- The min realistic value of ETo (mm).\n
    ...
    met_names = array(['R_n', 'R_s', 'G', 'T_min', 'T_max', 'T_mean', 'T_dew', 'RH_min', 'RH_max', 'RH_mean', 'n_sun', 'U_z', 'P_atmos'])
    ...
    #Air Pressure
    est_val.loc[df1['P_atmos'].isnull()] = est_val.loc[df1['P_atmos'].isnull()] + 1000000
    df1.loc[df1['P_atmos'].isnull(), 'P_atmos'] = 101.3*((293 - 0.0065*z_msl)/293)**5.26

    #Psychrometric constant
    gamma = (0.665*10**-3)*df1['P_atmos']
    ...
    if time_int == 'days':
        ...
        w = pi/12*(((hour_vec+0.5) + 0.6666667*(TZ_lg - lon) + S_c) - 12)
...
#    ETo.columns = ['ETo_FAO_mm', 'ETo_Har_mm', 'Est_val']
    ...
    ##ETo equation with filled holes using spline interpolation (use with caution)
    if fill:
        ETo_FAO_fill = ETo_FAO.interpolate('pchip', limit=maxgap)
        ETo_FAO_fill.name = 'ETo_Har_fill_mm'
