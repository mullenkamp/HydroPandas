...
def random_areas(poly, name_col, change_area, land_change, tol=0.05):
    ...
    -----------
    poly: GeoDataFrame
        Polygon GeoDataFrame of the parcels with a column associated with each study area.
    name_col: str
        The column in poly that contains the study are names.
    change_area: Series
        A Series with the area to be assigned with the index as the study area names.
    land_change: DataFrame
        A DataFrame of the percentage of each land use type associated with each study area name.
    tol: float
        The tolerance ratio of the change_area when assigning parcels. The cumulative parcels sizes must not be >+- the tolerance ratio.
